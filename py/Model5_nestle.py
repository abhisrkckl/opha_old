...
data = get_data(data_file="OJ287_real.txt")

# Define a function mapping the unit cube to the prior space.
# This function defines a flat prior in [-5., 5.) in both dimensions.
M = 20000000000.*MSun
t0  = 1886.05*year
x0min,x0max = 0.015, 0.02
e0min,e0max = 0.635,0.645
u0min,u0max = 0.33,0.375
t0min,t0max = t0+15*day,t0+160*day
Mmin,Mmax = M*0.5, M*1.5
etamin,etamax = 0.010,0.022
kSOmin,kSOmax = -0.1,0.1
d1min, d1max = 0.4,0.8
mins = np.array((x0min,e0min,u0min,t0min,Mmin,etamin,kSOmin,d1min))
maxs = np.array((x0max,e0max,u0max,t0max,Mmax,etamax,kSOmax,d1max))
spans = maxs-mins
def prior_transform(x):
        return spans*x + mins

result = run_sampler(model, prior_transform, data, z=0.306, npts=150)
